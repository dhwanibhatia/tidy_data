...
#filter out the indices and the names of columns which are either means or std dev
indices <- grep("mean|std",feature_name[,2])
values <- grep("mean|std",feature_name[,2],value = TRUE)

#Filter out the data corresponding to above filtered columns
X_train_data <- X_train[,indices]
#Assign column names to data from above features values i.e the names of the features
